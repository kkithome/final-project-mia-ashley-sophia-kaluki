    ...
    try {
      ...
      for (Map<String, Object> pin : allPins) {
        if (pin.get("user") != null && pin.get("user").equals(uid)) {
          ...
          if (docID != null) {
            DocumentReference docRef = db.collection("all_users").document(
                "pins").collection("pins").document(docID);

            docRef.delete().get(); System.out.println("Deleted document with ID: " + docID);
          }
