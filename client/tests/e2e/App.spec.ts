...
test('testing for error message when a non-brown.edu user tries to create an account', async ({ page }) => {
  ...
  //   signInParams: {
  //     strategy: "password",
  //     password: "testingpasswordjkjkj",
  //     identifier: "team@baist.ai",
  //   },
  // });
  // await page.getByText('You do not have permission to').click();
});
...
test('clicking going increases attendance by 1', async ({ page }) => {
  ...
  await expect(page.getByRole("main")).toContainText("1 Attending");
...
test('favoriting event adds it to the user profile favorited events list', async ({ page }) => {
  ...
  await expect(page.getByText("India Point Park").first()).toBeVisible();
  await page.getByRole('button', { name: 'Back to Main' }).click();
...
test('activity page elements exist', async ({page}) => {
  setupClerkTestingToken({ page });
  await page.goto("http://localhost:8000/");
  await clerk.loaded({ page });
  await page.locator("html").click();
  ...
  await expect(
    page.getByRole("heading", { name: "Brown Bee Coffee" })
  ).toBeVisible();

  await page.getByRole("heading", { name: "Brown Bee Coffee" }).click();
  await expect(page.getByRole("main")).toContainText(
    "A rare gem nestled in the heart of College Hill, Providence. Experience precision-poured coffee, with our Poursteady, and handmade croissants."
  );
  await expect(page.getByRole("main")).toContainText(
    "Date: Open Thursday-Monday"
  );
  await expect(page.getByRole("main")).toContainText("Time: 6:00am");
  await expect(
    page.getByRole("img", { name: "Brown Bee Coffee" })
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByRole("button", { name: "Back to main" })
  ).toBeVisible();
})

test('user profile page elements exist', async ({page}) => {
  setupClerkTestingToken({ page });
  await page.goto("http://localhost:8000/");
  await clerk.loaded({ page });
  await page.locator("html").click();
  ...
  await expect(page.getByRole('img', { name: 'null\'s profile' })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Anon' })).toBeVisible();
  await expect(page.getByRole('main')).toContainText('Favorited Events');
  await expect(page.getByRole('main')).toContainText('Upcoming Events');
  await expect(page.getByRole('button', { name: 'Back to Main' })).toBeVisible();
})